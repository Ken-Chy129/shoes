...
@Slf4j
@Component
public class StockXCustodialPriceDownTaskRunner implements Runnable {
        ...
        try {
            while (true) {
                try {
                    ...
                    Long taskId = TaskSwitch.CURRENT_STOCK_CUSTODIAL_PRICE_DOWN_TASK_ID;
                    if (taskId != null) {
                        taskMapper.updateTaskRound(taskId, TaskSwitch.CURRENT_STOCK_CUSTODIAL_PRICE_DOWN_ROUND);
                    }
                    ...
                    log.error("StockX寄存压价任务执行异常: {}", e.getMessage(), e);
                    Long taskId = TaskSwitch.CURRENT_STOCK_CUSTODIAL_PRICE_DOWN_TASK_ID;
                    if (taskId != null) {
    ...
    private boolean sleepWithCancelCheck(Long taskId) throws InterruptedException {
        long remaining = TaskSwitch.STOCK_CUSTODIAL_PRICE_DOWN_TASK_INTERVAL;
        while (remaining > 0) {
    ...
    private boolean detectCancelTask(Long taskId) {
        if (TaskSwitch.CANCEL_STOCK_CUSTODIAL_PRICE_DOWN_TASK) {
            log.info("StockX寄存压价任务已取消，终止执行");
            if (taskId != null) {
            ...
            TaskSwitch.CURRENT_STOCK_CUSTODIAL_PRICE_DOWN_TASK_ID = null;
