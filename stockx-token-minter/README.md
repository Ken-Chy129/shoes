...
| `browserProxy` | 可选 Playwright 浏览器代理，如 `socks5://127.0.0.1:18080`；仅浏览器流量走代理，写回后端仍直连 |
...
如果日志连续出现 `authorize_timeout`，同时授权请求为 `403 challenge`，可让服务器浏览器通过一台
长期在线的住宅网络 Mac 出口访问 StockX：

1. Mac 安装 `deploy/cn.ken.stockx-reverse-proxy.plist` 到 `~/Library/LaunchAgents/`，由 launchd 保持
   `127.0.0.1:18080` 的远端 SOCKS 隧道。
2. 服务器 `config.json` 设置 `"browserProxy": "socks5://127.0.0.1:18080"`。
3. 将 `deploy/sx-token-minter-retry.conf` 安装为 systemd service drop-in。一次刷新失败后会每 30 分钟重试，
   成功后停止重试，等待原有的 8 小时 timer。

Mac 睡眠或离线时代理会暂时不可用；任务会明确失败并持续重试，不会再显示假成功。
