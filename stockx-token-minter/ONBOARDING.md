...
- 服务器：发 token 机部署目录见运维记录，后端 API 在容器内 `127.0.0.1`（具体地址见运维记录，勿写入仓库）。
...
```bash
scp /tmp/seed_新账号.json root@<服务器>:<发token机目录>/
```
...
> 真实踩过的坑：某个 Chrome profile 的名字和它实际登录的 StockX 账号并不对应，
> 靠 uuid 核验才发现，否则会把 token 写错账号、操作到别的账号的库存。
