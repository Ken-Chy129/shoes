...
5. `update_bids`（修改出价）：上传包含「出价ID、价格」的 Excel，其中价格是最高可接受价；开启费率监控时还需逐行填写「费率配置是否启用」，任务循环检查并按需追价或压价。
...
创建出价和修改出价都可通过任务参数 `feeMonitorEnabled` 开启费率监控，默认关闭。开启后配置：

- `merchantFeeRate`：商家手续费率，默认 `0.07`。
- `minMerchantFee`：最低商家手续费，默认 `$5.79`。
- `transferFeeRate`：转账费率，默认 `0.03`。
- `processOutsideExcel`：仅修改出价使用，控制是否处理 Excel 外的当前有效出价。

当前现货标价取 standard 最低卖价，计算方式为：
...
金额按美分四舍五入，求购价等于可盈利上限时允许，严格超过时视为不盈利。创建出价遇到不盈利或缺少现货标价时跳过；修改出价仅对 Excel 行级开关为“是”的商品执行盈利判断，不盈利或缺少现货标价时改为 `$1`。行级开关为“否”时保留原有“市场最高价加 `$1`、不超过 Excel 最高价”逻辑。

修改任务同时开启 `processOutsideExcel` 后，也会处理 Excel 外的当前有效出价。这些商品没有 Excel 最高价限制：落后市场时按市场最高价加 `$1`，但候选价不盈利或缺少现货标价时改为 `$1`。任务每轮仍只分页读取一次全部有效出价，复用响应中的市场最高价和现货标价，不增加逐商品行情请求。

完整边界和测试策略见 `docs/specs/stockx-bid-fee-monitoring.md`。
